...
namespace DatabaseLib
{
    ...
    {      
        ...
        public void SaveLog(int type, string txt, string fileTo = "testSQL.txt")
        {
            ...
            StreamWriter streamWriter = new StreamWriter(@"..\..\..\" + fileTo, true, Encoding.GetEncoding("utf-8"));
            txt = "\n" + DateTime.Now.ToString("dd.MMM.yyyy HH:mm:ss")+ tMsg +"\n" + txt;
        ...
        public List<string> TableInfo(string tableName)
        {
            ...
            {
                columns.Add(row[0].ToString()); // row[1] первым столбцом идет cid, а не name.
            }
            return columns;
        }
            ...
            try
            {
                ...
                foreach (Parameter iparam in parameters)
                {
                    ...
                    if (ifFirst)
                    {
                        ...
                        //queryColumns.Append("@" + iparam.ColumnName);
                        //queryValues.Append(iparam.Value.ToString());
...
//                SaveLog(2, sql);
        ...
        #region Инициализация базы данных задач
        /// <summary>
        /// Инициализация структуры базы данных для приложения управления задачами
        /// </summary>
        public void InitializeTaskDatabase()
        {
            try
            {
                // Создание таблицы Subjects (предметы)
                string sql_subjects = @"CREATE TABLE IF NOT EXISTS Subjects (
                    Id INTEGER PRIMARY KEY AUTOINCREMENT,
                    Name TEXT NOT NULL UNIQUE
                );";

                // Создание таблицы Tasks (задания)
                string sql_tasks = @"CREATE TABLE IF NOT EXISTS Tasks (
                    Id INTEGER PRIMARY KEY AUTOINCREMENT,
                    Title TEXT NOT NULL,
                    Description TEXT,
                ...
                ExecuteNonQuery(new string[] { sql_subjects, sql_tasks });

                // Заполняем таблицу Subjects начальными данными
                InsertDefaultSubjects();

                // Создаем индексы для оптимизации
                string sql_index_deadline = "CREATE INDEX IF NOT EXISTS idx_tasks_deadline ON Tasks (Deadline)";
                string sql_index_subject = "CREATE INDEX IF NOT EXISTS idx_tasks_subject ON Tasks (SubjectId)";

                ExecuteNonQuery(new string[] { sql_index_deadline, sql_index_subject });

                // Логируем успешную инициализацию
                SaveLog(2, "База данных задач успешно инициализирована");
            }
            catch (Exception error)
            {
                lastError = string.Format("Ошибка при инициализации базы данных задач!\n{0}", error.Message);
                SaveLog(1, lastError);
            }
        }

        /// <summary>
        /// Заполнение таблицы Subjects начальными данными
        /// </summary>
        private void InsertDefaultSubjects()
        {
            try
            {
                // Ваши предметы: физика, ТРПО, АИС
                ...
                foreach (string subject in defaultSubjects)
                {
                    ...
                    // Используем существующий метод Insert
                    Insert("Subjects", parameters);
                }

                SaveLog(2, "Добавлены предметы по умолчанию: Физика, ТРПО, АИС");
            }
            catch (Exception error)
            {
                lastError = string.Format("Ошибка при добавлении предметов по умолчанию\n{0}", error.Message);
                SaveLog(1, lastError);
            }
        }
        #endregion
